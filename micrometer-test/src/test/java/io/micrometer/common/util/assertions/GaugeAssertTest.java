...
class GaugeAssertTest {
    ...
    void shouldFindGaugeByName() {
        Gauge.builder("foo", () -> 10.0)
            .register(simpleMeterRegistry);

        meterRegistryAssert.gauge("foo")
            .hasValue(10.0);
    }
    ...
    void shouldThrowIfGaugeNotFound() {
        ...
        assertThatThrownBy(() -> meterRegistryAssert.gauge("foo", Tag.of("other-tag", "xxx"))).isInstanceOf(AssertionError.class)
    ...
    void shouldFindGaugeByNameAndTags() {
        Gauge.builder("foo", 1.0, n -> n)
            .tag("tag-1", "aa")
            .register(simpleMeterRegistry);

        Gauge.builder("foo", 99.0, n -> n)
            .tag("tag-1", "bb")
            .tag("tag-2", "cc")
            .register(simpleMeterRegistry);

        meterRegistryAssert.gauge("foo", Tag.of("tag-1", "aa"))
            .hasValue(1.0);
        meterRegistryAssert.gauge("foo", Tag.of("tag-1", "bb"), Tag.of("tag-2", "cc"))
            .hasValue(99.0);
    }

    @Test
    void shouldLeverageDoubleAsserts() {
        Gauge.builder("foo", () -> 50.0)
            .register(simpleMeterRegistry);
